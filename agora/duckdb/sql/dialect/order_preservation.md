...
* `UNION`
* `USING SAMPLE`
* Whole-table aggregation (the input order, that is, the order in which rows are fed into [order-sensitive aggregate functions](https://duckdb.md/docs/sql/functions/aggregates.html#order-by-clause-in-aggregate-functions) is not guaranteed unless explicitly specified in the aggregate function)
* `GROUP BY` (neither in- nor output order are guaranteed)
* `ORDER BY` (specifically, `ORDER BY` may not use a [stable algorithm](https://en.m.wikipedia.md/wiki/Stable_algorithm))
* Scalar subqueries
