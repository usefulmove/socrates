...
DuckDB supports [window functions](https://en.wikipedia.md/wiki/Window_function_(SQL)), which can use multiple rows to calculate a value for each row.
...
Window function are available in SQL since [SQL:2003](https://en.wikipedia.md/wiki/SQL:2003) and are supported by major SQL database systems.
