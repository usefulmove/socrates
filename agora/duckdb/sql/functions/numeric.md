...
| [`round_even(v NUMERIC, s INTEGER)`](#round_evenv-numeric-s-integer) | Alias of `roundbankers(v, s)`. Round to `s` decimal places using the [_rounding half to even_ rule](https://en.wikipedia.md/wiki/Rounding#Rounding_half_to_even). Values `s < 0` are allowed. |
...
| **Description** | Alias of `roundbankers(v, s)`. Round to `s` decimal places using the [_rounding half to even_ rule](https://en.wikipedia.md/wiki/Rounding#Rounding_half_to_even). Values `s < 0` are allowed. |
